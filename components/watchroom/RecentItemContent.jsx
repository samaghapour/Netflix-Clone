...
const getMovieDetails = async (key) => {
  const keys = key.queryKey;
  let isMovie = keys[2] === 'true' ? 'movie' : 'tv';

  return axios
    .all([
      axios.get(
        `https://api.themoviedb.org/3/${isMovie}/${keys[1]}/videos?api_key=${apiKey}`
      ),
      axios.get(
        `https://api.themoviedb.org/3/${isMovie}/${keys[1]}?api_key=${apiKey}`
      ),
    ])
    .then((res) => {
      return res;
...
const RecentItemContent = ({ id, isMovie }) => {
  const { data, status } = useQuery(
    ['moviedata', id, isMovie],
    getMovieDetails
  );

  const details = Array.isArray(data) && data.length > 0 && data[1].data;
  const video =
    Array.isArray(data) &&
    data.length > 0 &&
  ...
  return (
    <div className='close watchBox-items-content' id='recentItemContent'>
...
export default RecentItemContent;
